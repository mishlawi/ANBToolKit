...
def changdir(rel,onto):
    cwd = os.getcwd()
    name = os.path.basename(cwd)

    
    if rel == 'gp 1':
        gpFoldersQres = onto.query(sparql_queries.gp_folderPath_Qres(name))[0]
        os.chdir(gpFoldersQres)
        
    elif rel == 'gp 2':
        gpFoldersQres = onto.query(sparql_queries.gp_folderPath_Qres(name))[1]
        os.chdir(gpFoldersQres)
